...
flags.DEFINE_string('video_dir', 'videos/intervention/one-chunk/gs_weight/auto_strength', 'Directory for videos.')
...
def get_intervention_vector(clf, direction):
    ...
    if opposite_idx == target_idx:
        ...
        class_index_in_matrix = np.where(clf.classes_ == target_idx)[0][0]
        opposite_index_in_matrix = np.where(clf.classes_ == opposite_idx)[0][0] #actually, we should try minus the straight vector
        
        weight_vector = clf.coef_[class_index_in_matrix]
        opposite_vector = clf.coef_[opposite_index_in_matrix]

        # gram_schmidt to make orthogonal to opposite direction
        proj_length = np.dot(weight_vector, opposite_vector) / np.dot(opposite_vector, opposite_vector)
        proj_vector = proj_length * opposite_vector
