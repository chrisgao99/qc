...
# CHANGED: Imported restore_agent to match your eval script
...
# CHANGED: Added load_dir and load_step flags
flags.DEFINE_string('load_dir', None, 'Path to the experiment directory containing checkpoints.')
flags.DEFINE_integer('load_step', 0, 'The specific checkpoint step to restore.')
...
def main(_):
    ...
    obs, info = env.reset()
    print("goal position in info:", info)
    print("current goal position :", env.unwrapped.cur_goal_xy)
    ...
    # CHANGED: Restore logic using restore_agent(agent, dir, step)
    if FLAGS.load_dir is not None:
        if FLAGS.load_step == 0:
            raise ValueError("Must provide --load_step when --load_dir is set.")
        
        print(f"Restoring agent from {FLAGS.load_dir} at step {FLAGS.load_step}...")
        agent = restore_agent(agent, FLAGS.load_dir, FLAGS.load_step)
